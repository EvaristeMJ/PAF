...
# lecture de donn�es sauvegard�es au format matlab LECTURE AEIOU.MAT
...
#Calculer l'écart entre les beats = tempo
...
def tempo_from_correl(cor):
    ...
    print(M)
    S = 0
    for k in range(6): #on moyenne les 4 premiers recouvrements dans l'autocorrélation
        S = S + M[k]/(k+1)
    S = S/6
...
"""
hop_length = 512
tempogram = librosa.feature.tempogram(y=audio_data, sr=sample_rate, hop_length=hop_length)
